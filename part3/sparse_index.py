...
class SparseDocument:
    """Documento indexado en el store sparse."""
...
def normalize_text(text: str) -> str:
    """Lowercase + remocion de tildes para emparejar variantes del espanol."""
...
def _parse_mdb_row(row: dict[str, Any]) -> dict[str, str]:
    """Soporta TSV normal y filas que el driver puede devolver aplastadas."""
    ...
    try:
        values = next(csv.reader(StringIO(raw_vals)))
    except Exception:
        values = str(raw_vals).split(",")
    return {key: _clean_value(value) for key, value in zip(keys, values)}
...
def rows_to_documents(rows: Iterable[dict[str, Any]]) -> list[SparseDocument]:
    ...
    for row in rows:
        ...
        chunk_id = data.get("emb.chunk_id") or data.get("chunk_id") or ""
        intervention_id = data.get("i.id") or data.get("id") or ""
        text = data.get("emb.content") or data.get("content") or ""

        doc_id = chunk_id or intervention_id
...
class BM25Index:
    """
    Indice BM25 propio y serializable.

    BM25 fue elegido porque premia coincidencias exactas, apellidos, siglas y
    numeros de boletin, justo el flanco debil esperado para el recuperador denso.
    """
    ...
    def from_mdb(cls, client: Any, limit: Optional[int] = None) -> "BM25Index":
        ...
        RETURN ?emb.chunk_id, ?i.id, ?emb.content, ?pos.name, ?pp.name
        """
        if limit:
            query += f"\nLIMIT {int(limit)}"
